...
class KotlinEvaluator(val codeFragment: KtCodeFragment, private val sourcePosition: SourcePosition?) : Evaluator {
    ...
    private fun compiledCodeFragmentDataK2(context: ExecutionContext): CompiledCodeFragmentData = runReadAction {
        ...
        analyze(codeFragment) {
            try {
                ...
                when (val result = compile(codeFragment, compilerConfiguration, compilerTarget)) {
    ...
    private fun computeCodeFragmentParameterInfo(result: KtCompilationResult.Success): K2CodeFragmentParameterInfo {
        ...
        for (capturedValue in result.capturedValues) {
            val parameter = capturedValue.toDumbCodeFragmentParameter()
    ...
    private fun CodeFragmentCapturedValue.toDumbCodeFragmentParameter(): CodeFragmentParameter.Dumb {
        return when (this) {
            ...
            is CodeFragmentCapturedValue.LocalFunction ->
                CodeFragmentParameter.Dumb(CodeFragmentParameter.Kind.LOCAL_FUNCTION, name)
